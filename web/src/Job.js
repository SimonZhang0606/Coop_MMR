...
export default class Company extends React.Component {
  ...
  handleSubmit(e) {
    ...
    fetch(
      `http://localhost:5000/jobs/${this.state.jid}/reviews`,
      requestOptions
    ).then((response) => response.json());
  }
  ...
  render() {
    return (
      <>
        ...
        <p>{`Rating: ${
          this.state.job_avg_rating ? this.state.job_avg_rating : "N/A"
        }`}</p>
        <p>{`Salary: ${
          this.state.job_avg_salary ? this.state.job_avg_salary : "N/A"
        }`}</p>
        <p>{`Max salary: ${
          this.state.job_max_salary ? this.state.job_max_salary : "N/A"
        }`}</p>
        <p>{`Min salary: ${
          this.state.job_min_salary ? this.state.job_min_salary : "N/A"
        }`}</p>
        ...
        <form onSubmit={(e) => this.handleSubmit(e)}>
          <label>
            ...
            <input
              ...
              type="text"
              name="summary"
              onChange={(e) => this.handleChange(e, "review_headline")}
            />
            <br></br>
          </label>
          <label>
            ...
            <input
              ...
              type="number"
              name="rating"
              onChange={(e) => this.handleChange(e, "review_rating")}
            />
            <br></br>
          </label>
          <label>
